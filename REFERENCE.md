...
| [`DCM`](inc/damp/math/geometry.hpp#L63) | Direction cosine matrix — @f$ 3 \times 3 @f$ rotation (SO(3) wrapper over Mat3) |
...
| [`Quaternion`](inc/damp/math/geometry.hpp#L381) | Unit quaternion rotation @f$ (w, x, y, z) @f$ (Hamilton product) |
| [`Transform4`](inc/damp/math/geometry.hpp#L826) | @f$ 4 \times 4 @f$ homogeneous transform (SE(3) interop / DH export) |
...
| [`ceil`](inc/damp/math/math.hpp#L328) | Ceiling — smallest integer ≥ x |
| [`copysign`](inc/damp/math/math.hpp#L403) | Copy sign — magnitude of mag with the sign of sgn_src |
| [`cos`](inc/damp/math/math.hpp#L169) | Cosine |
| [`db2mag`](inc/damp/math/math.hpp#L467) | Decibels to magnitude, 10^(db/20) |
| [`deg2rad`](inc/damp/math/math.hpp#L489) | Degrees to radians, deg·π/180 |
| [`exp`](inc/damp/math/math.hpp#L231) | Exponential function |
| [`finite_non_negative`](inc/damp/math/math.hpp#L445) | True if x is finite and non-negative (@f$ x \ge 0 @f$) |
| [`finite_positive`](inc/damp/math/math.hpp#L435) | True if x is finite and strictly greater than zero |
| [`floor`](inc/damp/math/math.hpp#L316) | Floor — largest integer ≤ x |
| [`fmod`](inc/damp/math/math.hpp#L358) | Floating-point remainder, x − y·trunc(x/y) (sign of x), matching std::fmod's truncated-quotient convention |
| [`hypot`](inc/damp/math/math.hpp#L76) | Euclidean distance hypot(x, y) = √(x² + y²), without overflow |
| [`isfinite`](inc/damp/math/math.hpp#L422) | Finiteness test — false for NaN and ±∞ |
| [`log`](inc/damp/math/math.hpp#L248) | Natural logarithm |
| [`log10`](inc/damp/math/math.hpp#L377) | Base-10 logarithm, log10(x) = ln(x) / ln(10) |
| [`mag2db`](inc/damp/math/math.hpp#L456) | Magnitude to decibels, 20·log10(mag) |
| [`nearbyint`](inc/damp/math/math.hpp#L342) | Round to nearest integer. Runtime follows the backend (round half to even); the compile-time path rounds ties away from zero — immaterial for range reduction |
| [`pow`](inc/damp/math/math.hpp#L267) | Power function, base^exponent (+1 more overload) |
| [`rad2deg`](inc/damp/math/math.hpp#L478) | Radians to degrees, rad·180/π |
| [`sgn`](inc/damp/math/math.hpp#L394) | Sign function — −1 if val < 0, 1 if val > 0, 0 if val == 0 |
...
| [`tan`](inc/damp/math/math.hpp#L214) | Tangent |
| [`wrap`](inc/damp/math/math.hpp#L506) | Wrap x into the half-open interval [min, max) (period max − min) |
| [`wrap_pi`](inc/damp/math/math.hpp#L541) | Wrap an angle to @f$ [-\pi,\pi) @f$ |
| [`wrap_two_pi`](inc/damp/math/math.hpp#L559) | Wrap an angle to @f$ [0, 2\pi) @f$ |
...
| [`QRDecomposition`](inc/damp/matrix/decomposition.hpp#L191) | Thin QR factorization @f$ A = Q R @f$ (modified Gram–Schmidt) |
...
| Name | Description |
| ---- | ----------- |
| [`backward_substitute_transpose`](inc/damp/matrix/solve.hpp#L59) | Backward substitution for @f$ L^\top x = b @f$ (real) / @f$ L^{\mathrm{H}} x = b @f$ layout |
| [`cholesky`](inc/damp/matrix/decomposition.hpp#L82) | Cholesky decomposition for positive-definite matrices |
| [`cholesky_solve`](inc/damp/matrix/solve.hpp#L159) | Solve @f$ A X = B @f$ via Cholesky (@f$ A = L L^{\mathrm{H}} @f$) |
...
| [`cosh`](inc/damp/matrix/functions.hpp#L800) | Matrix hyperbolic cosine @f$ \cosh(A) = \bigl(\exp(A) + \exp(-A)\bigr)/2 @f$ |
...
| [`det`](inc/damp/matrix/functions.hpp#L176) | Matrix determinant @f$ \det(A) @f$ |
| [`expm`](inc/damp/matrix/functions.hpp#L317) | Matrix exponential via scaling and squaring with Padé approximant of degree 13 |
| [`forward_substitute`](inc/damp/matrix/solve.hpp#L37) | Forward substitution for @f$ L x = b @f$ |
| [`frobenius_norm`](inc/damp/matrix/functions.hpp#L92) | Frobenius norm @f$ \\|A\\|_F = \sqrt{\sum_{ij} \|a_{ij}\|^2} @f$ |
| [`full_qr`](inc/damp/matrix/decomposition.hpp#L291) | Full QR factorization via Householder reflections (real or complex T) |
| [`infinity_norm`](inc/damp/matrix/functions.hpp#L35) | Infinity norm @f$ \\|A\\|_\infty @f$: maximum absolute row sum |
| [`inverse`](inc/damp/matrix/solve.hpp#L248) | Matrix inverse @f$ A^{-1} @f$ via mat::solve(@f$ A, I @f$) |
...
| [`lu_solve`](inc/damp/matrix/solve.hpp#L187) | Solve @f$ A X = B @f$ via LU with partial pivoting |
...
| [`one_norm`](inc/damp/matrix/functions.hpp#L64) | One-norm @f$ \\|A\\|_1 @f$: maximum absolute column sum |
...
| [`sinh`](inc/damp/matrix/functions.hpp#L784) | Matrix hyperbolic sine @f$ \sinh(A) = \bigl(\exp(A) - \exp(-A)\bigr)/2 @f$ |
| [`solve`](inc/damp/matrix/solve.hpp#L82) | Solve lower-triangular system @f$ L X = B @f$ via forward substitution (+2 more overloads) |
...
| [`two_norm`](inc/damp/matrix/functions.hpp#L122) | Spectral norm (2-norm): largest singular value of @f$ A @f$ |
...
| [`feedback`](inc/damp/systems/state_space.hpp#L311) | Negative feedback: @f$ y = \mathrm{sys1}(u - \mathrm{sys2}(y)) @f$ (+1 more overload) |
...
| [`series`](inc/damp/systems/state_space.hpp#L196) | Series connection: sys2 follows sys1 (@f$ u \to \mathrm{sys1} \to \mathrm{sys2} \to y @f$) (+1 more overload) |
...
| [`subtract`](inc/damp/systems/state_space.hpp#L394) | Differencing connection: outputs @f$ y = y_1 - y_2 @f$ |
...
| [`controllability_gramian`](inc/damp/design/stability.hpp#L118) | Continuous/discrete controllability Gramian @f$ W_c @f$ |
...
| [`dlyap`](inc/damp/design/lyapunov.hpp#L130) | Solve the discrete-time Lyapunov (Stein) equation @f$ A X A^\top - X + Q = 0 @f$ |
...
| [`lyap`](inc/damp/design/lyapunov.hpp#L105) | Solve the continuous-time Lyapunov equation @f$ A X + X A^\top + Q = 0 @f$ |
...
| [`observability_gramian`](inc/damp/design/stability.hpp#L146) | Continuous/discrete observability Gramian @f$ W_o @f$ |
...
| [`project_affine`](inc/damp/controllers/action_governor.hpp#L231) | Project u_des onto the polyhedron @f$A u \le b@f$ (Euclidean) |
...
| Name | Description |
| ---- | ----------- |
| [`ActionGovernor`](inc/damp/controllers/action_governor.hpp#L331) | Runtime action governor — QP projection of @f$u_{\mathrm{des}}@f$ onto @f$A u \le b@f$ |
...
| [`ins_inject`](inc/damp/estimation/ins_eskf.hpp#L430) | Inject @f$ \delta x @f$ into the nominal INS state (right-multiplicative @f$ q @f$) |
...
| [`ins_update_position`](inc/damp/estimation/ins_eskf.hpp#L548) | Sparse position update: @f$ y = p + R\ell @f$ (+1 more overload) |
| [`ins_update_velocity`](inc/damp/estimation/ins_eskf.hpp#L594) | Sparse velocity update: @f$ y = v @f$ at the IMU origin (no lever-arm) |
...
| [`rows`](inc/damp/estimation/ins_eskf.hpp#L345) | Sparse @f$ y = F x @f$ for the 15-state INS first-order structure (G = I path) |
...
| [`continuous_lpf_exact_step`](inc/damp/filters/lowpass.hpp#L37) | Exact step of continuous first-order LPF @f$ \dot y = -\omega_c (y - u) @f$ |
...
| Name | Description |
| ---- | ----------- |
| [`Jet`](inc/damp/trajectory/trajectory_types.hpp#L95) | Truncated jet of a scalar motion sample: @f$ d[k] = s^{(k)} @f$ |
...
| [`inverse_deadband_sine`](inc/damp/toolbox/conditioning.hpp#L152) | Sine-shaped inverse dead zone (full boost once @f$\|x\|\ge w@f$) |
...
| [`soft_sign`](inc/damp/toolbox/conditioning.hpp#L103) | Smooth unit direction @f$ x / \sqrt{x^{2}+\varepsilon^{2}} \in (-1,1) @f$ |
...
| Name | Description |
| ---- | ----------- |
| [`arange`](inc/damp/analysis/linspace.hpp#L134) | Half-open arithmetic range @f$ [\mathrm{start},\,\mathrm{stop}) @f$ with step step (+2 more overloads) |
...
| [`norm_hinf`](inc/damp/analysis/norms.hpp#L134) | H∞ norm of a state-space system: @f$ \sup_\omega \bar\sigma\,G(j\omega) @f$ |
...
| [`dlyap`](inc/damp/matlab.hpp#L1062) | MATLAB® alias for the discrete Lyapunov solve @f$ AXA^\top-X+Q=0 @f$ |
...
| [`lqry`](inc/damp/matlab.hpp#L680) | Output-weighted continuous LQR (state cost @f$Q = C^\top Q_y C@f$) (+1 more overload) |
| [`lyap`](inc/damp/matlab.hpp#L1052) | MATLAB® alias for the continuous Lyapunov solve @f$ AX+XA^\top+Q=0 @f$ |
| [`make1DOF`](inc/damp/matlab.hpp#L220) | Force 1-DOF setpoint weights on a PID design result (@f$b=c=1@f$) |
...
| [`pade`](inc/damp/matlab.hpp#L998) | First-order Padé approximation of pure delay @f$e^{-sT}@f$ (+1 more overload) |
| [`pade2`](inc/damp/matlab.hpp#L1020) | Second-order Padé approximation of pure delay @f$e^{-sT}@f$ (+1 more overload) |
...
| [`damp/math/constexpr_math.hpp`](inc/damp/math/constexpr_math.hpp) | Compile-time scalar math: series / Newton-Raphson implementations |
